...
def find_strid_projid_annotlabel(excel_data_corpus):
    ...
    for row_data in excel_data_corpus:
        if row_data != None:
            tweet =  row_data['str_id'], row_data['project_id'],  row_data['tweet_text'], row_data['label'],
...
def shared_strid(corpus_tweets, bot_tweets):
    ...
    for number in standardize(corpus_tweets):
        if number in standardize(bot_tweets):
            for str_id, project_id, tweet_text, label in corpus_tweets:
                if str_id == number:
            #print(number)
                    shared_numbers_tuple.append((str_id, project_id, tweet_text, label))
...
def export_bot_tweets_corpus(shared_numbers_tuple, file_name):
    with open(file_name, 'w', newline='') as f:
        writer = csv.writer(f)        
        # write headers
        writer.writerow(['str_id', 'project_id', 'label'])
...
if __name__ == '__main__':
    ...
    workbook_bots = load_workbook('./botindex_strid__sources_scarcely_tweets.xlsx')
    workbook_corpus = load_workbook('./FINAL-CORPUS-ExpandedFull.xlsx')

    sheet_bots = workbook_bots['Sheet 1']
    sheet_corpus = workbook_corpus['Scarcely']
    ...
    #print(bot_strid)
    export_bot_tweets_corpus(bot_strid, 'bot_corpus_scarcely.csv')
